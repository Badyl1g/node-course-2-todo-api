...
describe('GET /todos/:id', () => {
  ...
  it('should return 404 if todo not found', done => {
    const falseID = new ObjectID().toHexString(); // create new id that does not exist in mock data
...
describe('DELETE /todos/:id', () => {
  ...
  it('should remove a todo', done => {
    const hexId = todos[1]._id.toHexString(); // copy id of the second todo
  ...
  it('should not remove a todo of other user', done => {
    const hexId = todos[0]._id.toHexString(); // copy id of the second todo
...
describe('POST /users', () => {
  ...
  it('should create a user', done => {
    ...
    request(app)
      ...
      .expect(200)
      .expect(res => {
        expect(res.headers['x-auth']).toBeTruthy(); // because of the '-'
        expect(res.body.email).toBe(email);
      })
      .end(err => { // using custom done() to check what has been saved to database
        if (err) {
          return done(err);
        }
